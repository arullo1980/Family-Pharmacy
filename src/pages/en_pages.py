...
<p>We use no advertising trackers, analytics scripts or social-media pixels. Map tiles are loaded from Esri (ArcGIS Online) or OpenStreetMap servers, which receive your IP address when requested.</p>
...
<h2>5. Intellectual property</h2><p>Text, graphics, code and the station dataset are © Wallet Partners LLC unless otherwise stated. Basemap © Esri and contributors; OpenStreetMap data © its contributors (ODbL). Fuel brands belong to their owners and are cited only to identify locations, implying no affiliation or endorsement.</p>
...
<h2>Third-party content</h2><p>The interactive map loads tiles from Esri (ArcGIS Online) or OpenStreetMap. Those requests reveal your IP address to the provider but set no cookies on this site.</p>
...
<h2>Content and images</h2><p>Station data was compiled from public listings on {ctx['DATA_DATE']}. Photographic images on this site are computer-generated illustrations and do not depict specific stations, people or brands. Basemap © Esri and contributors; OpenStreetMap data © its contributors (ODbL).</p>
