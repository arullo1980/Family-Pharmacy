...
def body(ctx):
    return f"""
<div class="page-title"><div class="wrap"><span class="eyebrow">Nosotros</span><h1>Wallet Partners LLC</h1><p>Una empresa de pagos con un solo objetivo: que las estaciones de combustible de la República Dominicana acepten tarjetas en condiciones que se ajusten a su negocio.</p></div></div>
...
<section class="alt"><div class="wrap"><div class="grid grid-3">
  <div class="card">{icon("doc")}<h3>Entidad</h3><p>Wallet Partners LLC, sociedad de responsabilidad limitada constituida en Estados Unidos. Las operaciones en el país se realizan a través de una filial registrada localmente según lo exija la estructura de patrocinio y la ley dominicana.</p></div>
  ...
  <div class="card">{icon("shield")}<h3>Estado</h3><p>Prelanzamiento. Estamos seleccionando el banco patrocinador y registrando las estaciones de la primera ola. Wallet Partners no es un banco y no retiene fondos de los comercios fuera de la cuenta del programa controlada por el banco.</p></div>
</div></div></section>
"""
